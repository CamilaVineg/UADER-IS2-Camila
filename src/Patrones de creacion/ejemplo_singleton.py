...
class Configuracion:
    ...
    _lock = Lock()  # para hacerlo thread-safe
                    #Lock hace que cuando está ejecutando el bloque, ningun otro hilo pueda entrar a ejecutar ese bloque.
    def __new__(cls, *args, **kwargs):
        if cls._instancia is None:
            with cls._lock:
                if cls._instancia is None:  # doble verificación para evitar que haya dos instancias queriendo acceder al bloque al mismo tiempo
